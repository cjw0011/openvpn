...
function newClient() {
	...
	# 输出目录：Web UI 可访问
	outputDir="/opt/openvpn/clients"
	mkdir -p "$outputDir"

	# 基于模板生成初始 ovpn
	grep -vE '^\s*(cert|key)\s+' /etc/openvpn/client-template.txt > "$outputDir/$CLIENT.ovpn"

	# 嵌套所有区块（含 cert/key/tls-crypt）
	{
	  echo "<ca>"
	  ...
	  echo "</tls-crypt>"
	} >> "$outputDir/$CLIENT.ovpn"

	# ✅ 删除 cert/key 段，只保留 ca 和 tls-crypt
	awk '
	/<cert>/ { skip=1 }
	/<\/cert>/ { skip=0; next }
	/<key>/ { skip=1 }
	...
	skip==1 { next }
	{ print }
	' "$outputDir/$CLIENT.ovpn" > "$outputDir/$CLIENT.cleaned.ovpn"

	mv "$outputDir/$CLIENT.cleaned.ovpn" "$outputDir/$CLIENT.ovpn"

	# 生成 .p12
	cp "/etc/openvpn/easy-rsa/pki/private/$CLIENT.key" "$outputDir/$CLIENT.key"
	cp "/etc/openvpn/easy-rsa/pki/issued/$CLIENT.crt" "$outputDir/$CLIENT.crt"
	cp "/etc/openvpn/easy-rsa/pki/ca.crt" "$outputDir/ca.crt"

	if [[ -z "$P12_PASSWORD" ]]; then
	  openssl pkcs12 -export \
		-inkey "$outputDir/$CLIENT.key" \
		-in "$outputDir/$CLIENT.crt" \
		-certfile "$outputDir/ca.crt" \
		-out "$outputDir/$CLIENT.p12" \
		-name "myvpn" \
		-passout pass:
	else
	  openssl pkcs12 -export \
	...
	# ✅ 可选：清理中间文件（保留 .ovpn 和 .p12）
	rm -f "$outputDir/$CLIENT.key" "$outputDir/$CLIENT.crt" "$outputDir/ca.crt"

	echo ""
	echo "✅ OVPN 配置已生成（无 cert/key 嵌套）: $outputDir/$CLIENT.ovpn"
	echo "✅ P12 文件已生成: $outputDir/$CLIENT.p12"
...
function revokeClient() {
	...
	rm -f "/opt/openvpn/clients/$CLIENT.p12"

	echo ""
	echo "✅ 用户 $CLIENT 吊销完成，相关文件已清除。"
}
...
function removeOpenVPN() {
	...
	if [[ $1 == "--new-client" && -n $2 ]]; then
	  ...
	  if [[ ! "$CLIENT" =~ ^[a-zA-Z0-9_-]+$ ]]; then
		echo "❌ 客户端名称非法，只允许英文、数字、下划线、短横线"
		exit 1
	  fi

	  # 检查是否已存在（包括吊销）
	  if grep -q "/CN=${CLIENT}" /etc/openvpn/easy-rsa/pki/index.txt; then
		echo "检测到 ${CLIENT} 已存在或已吊销，正在清理旧证书..."
	...
	# Web UI 参数支持 - 吊销用户（跳过交互）
	if [[ $1 == "--revoke-client" && -n $2 ]]; then
	  CLIENT=$2
	  cd /etc/openvpn/easy-rsa || exit 1
